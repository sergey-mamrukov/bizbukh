...
def addevent():
    ...
    if request.method == 'POST':
        ...
        try:
            # controlorgan,vidotchet,
            addEvent(event_name,data_start,data_end,eventTags,eventNalogs,eventOpfs,type_event,shortname)
...
def eventedit(eventid):
    ...
    if request.method == 'POST':
        try:
            editEvent(event, event_name, data_start, data_end,  eventTags, eventNalogs, eventOpfs, type_event,shortname)
            return redirect(url_for('event.eventlist'))
        except:
            print('error')
